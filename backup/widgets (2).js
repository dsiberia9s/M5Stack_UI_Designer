...
$.widget("namespace.UIRangebox", {
	...
	_setOption: function(key, value) {	
		...
		else if (key === "KEYBOARD_KEY_CODE")
		{
			...
			if (keyCode == 39)
			{
				properties.value += properties.step;
				element.trigger("eventFamily", [widget, properties]); // показать свойства
			} 
			else if (keyCode == 37)
			{
...
$.widget("namespace.FMValue", {
	options: {
		label: "Insert value",
		icon: "images/insert_32x32.png",
		fields: [
			{
				title: "From",
				source: "#m5_screen article",
				destination: "from",
				type: "<select>"
			},
			{
		...
		methods: [
			{
				...
				[
					{
						title: "Before",
						algorithm: "destination = source + destination;"
					},
					{
						title: "Insted",
	...
	_create: function() {
		...
		if (properties.blank_)
		{
			...
			var img = $("<img>", {src: properties.icon}).appendTo(td1);
			var td2 = $("<td>").appendTo(tr);
			var span = $("<span>", {text: properties.label}).appendTo(td2);
			return;
		}
